...
public:
    ...
    void receiveAccept(const AcceptData &d) {
        ...
        total_mutex.lock();
        total1 += gettime() - times[pro.id];
        printf("%lld %lf\n", total1, 1.0 * total1 / (pro.id + 1));
        total_mutex.unlock();
        ...
        total_mutex.lock();
        total2 += gettime() - times[pro.id];
        printf("%lld %lf\n", total2, 1.0 * total2 / (pro.id + 1));
        total_mutex.unlock();
    }
    ...
    void receiveCommit(const CommitData &d) {
        ...
        total_mutex.lock();
        total += gettime() - times[pro.id];
        printf("%lld %lf\n", total, 1.0 * total / (pro.id + 1));
        total_mutex.unlock();
    }
