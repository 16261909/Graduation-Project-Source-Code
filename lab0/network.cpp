...
void client_search(int i) {
    printf("Trying to connect: %d %d %d %d\n", i, isconn[6], isconn[62], isconn[117]);
    ...
    {
        cout << "Success connected to " << i << endl;
        isconn[i] = 1;
    }
}
...
void server_recv(int id) {
    ...
    while (true) {
        ...
        if (buffer[0] == 0) {
            ...
            pre.print();
            acceptor[pre.id].receivePrepare(pre);
        } else if (buffer[0] == 1) {
            PromiseData pro;
            ...
            acc.print();
            acceptor[acc.id].receiveAccept(acc);
        } else if (buffer[0] == 3) {
            AcceptedData acc;
            ...
            com.print();
            acceptor[com.id].receiveCommit(com);
        }
...
void server_init() {
    ...
    isconn[lip] = 1;
    localhost = lip;
    for(int i = 0; i < 20; i++)acceptor[i].init(i);
...
void for_client_send(char *msg) {
    ...
    for (auto ipstr: iplist) {
        int ip = getlow_s(ipstr);
        if (isconn[ip] == 0 || ip == localhost)continue;
